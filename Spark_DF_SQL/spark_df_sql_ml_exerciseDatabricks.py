...
# spark sql to get the top 'bachelor_rate' by 'education' group
# Check row count
spark.sql("SELECT COUNT(*) FROM adult").show()

# Check distinct marital_status values
spark.sql("SELECT DISTINCT marital_status FROM adult").show()
...
from pyspark.sql.functions import trim, col, avg, when, round

# Step 1: Register the table as a DataFrame
df_adult = spark.table("adult")

# Step 2: Perform the aggregation
df_result = df_adult.groupBy("education") \
    .agg(
        round(
            avg(
                when(trim(col("marital_status")) == "Never-married", 1).otherwise(0)
            ), 2
        ).alias("bachelor_rate")
    ) \
    .orderBy(col("bachelor_rate").desc())

# Step 3: Show top result
df_result.show(10)
#df_adult.groupBy("education").count().show()
...
# Question 3 - GBTClassifier - training Data 

from pyspark.ml.classification import GBTClassifier

# Proper instantiation
gb = GBTClassifier()
gb.setLabelCol("label")
gb.setFeaturesCol("features")
gb.setMaxIter(10)
...
# MAGIC %md
# MAGIC Looks like GBT or RF Models are blocked by the security policy
...
lrCvFeatureCoefficient = pd.DataFrame([(name, lrCvModel.bestModel.coefficients[idx]) for idx, name in attrs], columns = ['feature_name','feature_coefficient'])
print(lrCvFeatureCoefficient.sort_values(by=['feature_coefficient'], ascending = False))
